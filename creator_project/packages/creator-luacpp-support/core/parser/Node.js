...
class Node {
	...
	add_property_inttttt(newkey, value, data) {
		if (value in data) {
			let i = data[value];
			this._properties[newkey] = i;
		}
	...
	//leon: 這段會讓 this._properties 新增這些東西
	...
	//	this.add_property_vec2('scale', '_scale', data); //leon: 加這個會有一堆東西要改 (VT_SCALE*, c++ reader, fbs, AnimProp, ...)
	//	this.add_property_inttttt('scaleX', '_scaleX', data);
	//	this.add_property_inttttt('scaleY', '_scaleY', data);

		this.add_property_inttttt('scaleX', 'x', data._scale); //leon
		this.add_property_inttttt('scaleY', 'y', data._scale); //leon
